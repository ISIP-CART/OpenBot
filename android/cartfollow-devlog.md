...
## 2026-09-05：旧协议跟随修复候选 follow-r3-preflight-v1

- 视觉障碍判断复用正向坐标转换并裁剪越界框；按本帧轨迹绑定排除目标，旧路径按检测 id 与几何共同匹配。实车界面分别显示目标可见性与运动阻止原因。
- 原目标恢复上下文最多 1000 ms，不被重复漏检续期。单目标、原轨迹、默认空间 gate 通过的三个新鲜观测才恢复，先限 14 档且禁止外观学习；竞争、跳变和超时退回严格验证。恢复后需再次取得绑定本帧的强身份依据，才解除恢复限速和学习冻结。
- 同向对准停稳 350 ms，前进转自转及反向仍为 650 ms；脉冲上限 300 ms，接近中心提前制动、调度器独立超时保留。离开短等待也不能绕过反向截止时间。
- 距离选档后按原始误差限档：超过 .35 限 18，超过 .55 限 14；谨慎状态及身份限速优先。14/18/21 档最大内轮减档 6/8/10，最低内轮 6；实车与模拟器共用规则。
- 定向搜索可使用丢失前 500 ms 内本帧绑定、可靠且允许运动的连续跟踪证据；多人歧义、向外轨迹、角度、时限、陀螺仪及物理制动门控保留。
- 诊断记录候选参数版本及等待时间、恢复窗口和内轮减档。语音模块和固件未修改。三路协议交付下位机同学，新解析与实车传感器决策尚未启用。

实车验收（待操作，单元测试不代表实车成功）：

| 场景 | 操作与通过条件 | 结果 |
|---|---|---|
| 多人 | 目标静止／行走，旁人侧方经过三次；侧方不误拦，中央人员仍拦停 | 待测 |
| 转身／短漏检 | 原目标单人转身三次；符合窗口时第三有效观测后允许受限跟随，不更新外观库 | 待测 |
| 静止偏移 | 左右偏移各三次；同向对准更快，无持续交替振荡 | 待测 |
| 直线与缓弯 | 同一路线、步速各三次；对照旧版缓弯末端图像高度比例误差，确认拉距减少 | 待测 |
| 丢失与搜索 | 离开视野立刻取消前进，仅有界原地搜索，超时／急停停车 | 待测 |

回退：振荡复现将同向等待恢复 650 ms；弯道不稳定恢复旧弯道限档与减档 4/5/6，保留已验证的坐标和身份修复。按本轮约定，实车第一阶段通过后再提交 OpenBot 与主仓库指针。

离线验证：`:robot:check :robot:assembleDebug --offline` 成功；Debug / Release 各 353 项测试、0 失败（新增 16 项）。Lint 仍报告既有 10 项代码错误与 1 项检查器异常，675 警告、6 信息，不能将 check 成功解释为 lint 零问题。R3 黄金报文字段数、ASCII 和长度校验通过；固件队列与压力测试由下位机实现后验证。APK 未安装，实车待测。

APK SHA-256：`26c5b50da5d37b6bb0082375380e19fec16fda04982577c29e1ca91fcbb6a597`。

## Voice Cart Simulator：状态驱动语音引导（2026-09-05）

`Voice Cart Simulator` 已由独立的 TTS 试听页升级为独立模拟器入口：页面复用既有的
Human Cart Simulator 相机、检测、ReID、距离标定、搜索和模拟控制链，但没有修改原
Cart Simulator、公共状态机或真实小车控制路径。语音实现位于 `cartfollow/voice/`，使用
已在手机断网验证可用的 Android 系统简体中文 TTS，不新增 SDK、权限或网络依赖。

语音只在状态变化时播报：开始采集、目标待确认、确认后的距离标定、裁切时的后退提示、
重识别、倒计时、开始跟随、身份不确定、目标丢失、原地搜索与终态停止。普通引导排队播放，
身份不确定、丢失与 STOP 使用高优先级播放并打断过期提示。相同状态不逐帧重复；裁切提示
独立限频为 8 秒，避免被进入标定时的通用提示压制。暂停页面时停止并释放 TTS。

本轮没有自动确认照片或改变初始化条件；确认仍由用户在确认面板完成，因而可继续验证
“一个人从车前走到手机旁确认、再回到标定位置”的实际体验。语音解释当前步骤，不应被视为
车辆安全动作的替代品。

JDK 17 下使用本机已缓存的 AAR 和模型资源完成 Debug APK 与完整单测：Debug／Release
各 **340/340**，0 失败、0 错误、0 跳过。worktree 的 Git 版本号插件不识别 worktree
元数据文件，因此构建仅跳过版本号生成任务并使用本地已有版本号文件；其余编译、资源打包和
测试正常完成。Debug APK 为 `android/robot/build/outputs/apk/debug/robot-debug.apk`，
41,480,411 字节，SHA256 `5EBEC13FBF4FA1024E959FFF5C7FA0C0FEBB3736A3B2DA7B43C4E9F3E2A5F215`。

合并记录：2026-09-05 将语音工作树未提交改动合入当前工作区；保留旧协议跟随修复。本轮仅做文件与资源检查，未构建 APK，以上语音测试数据属于源工作树验证记录。


## 2026-09-05：缓弯响应候选 follow-visible-curve-v2（待实车验收）

根据 19:15:35 缓弯记录，约 .15 横向误差仍等速直行，64 秒附近 .6261 触发停转并持续拉距。本轮保留多人及短时恢复修复，调整如下：

- 转向只取原始观测：超过 .06 激活、.03 内退出；预测仅用于提前制动。移除控制末端 demand<=10 的直行覆盖，激活时至少一档差速。内轮减档采用 sqrt((abs(error)-.03)/.82) 增益，保留 6/8/10 基础减档、内轮最低 6、现有距离和谨慎限速。
- 距离 TOO_FAR 时直到 .85 才原地对准，回到 .55 退出；距离 OK 时 .10 进入、.04 退出，不向前追赶。
- 可见目标对准按误差分 5/8/10 档，同一次只能降档；单次最多 600 ms，停 150 ms 并要求新观测。前进转原地与反向仍等待 650 ms，实车 400 ms 失帧停车独立保留。
- 日志版本 8：曲线判定原因与有效 demand 来自同一观测，记录原始误差、轮差、观测至提交耗时；固件错误列增加原始接收时刻，UI 明确它是历史错误而不是当前帧回执。命令提交不代表底盘执行。
- 文案与语音改称视觉参考标定；TargetMemory 的视觉参考尺度与未来 desiredFollowingDistanceMm 分开，后者默认 null。头脚均出画面的近距控制等 R3 联调验证中央测距覆盖目标后再启用，旧最小测距不参与目标距离决策。

本轮不构建 APK、不安装、不烧录；旧 APK 不含本轮更改。实车待同路线、同速度重复三次静止横移／左右缓弯／多人，观察更早差速、减少停转与拉距，且无持续左右振荡。振荡时先回退增益及对准时长，保留小偏差判定和多人修复。实车通过后再提交代码和子模块指针。

本轮验证：`:robot:testDebugUnitTest :robot:testReleaseUnitTest --offline` 成功，Debug / Release 各 362 项，0 失败／错误／跳过。包括合入的语音测试、缓弯数值回放、转向滞回、对准分档／降档／超时／反向等待、失帧停车、多人和恢复回归，以及视觉／米制距离分离与错误时间列检查。XML 与 git diff --check 通过；未运行 assemble 或 APK 安装，未运行本轮 lint。


## 2026-09-05：直线振荡收敛候选 follow-curve-damped-v3（待实车验收）

20:10 旧协议实测中出现前进差速左右反向放大：误差 -.0833 时 c15,18，随后 .3366 时 c21,13，再到 -.6811 时 c7,14，最后出画。保存强度为 135%；未见本次新固件拒绝。目标轮速非线性和底盘响应可能放大高增益影响，缺少实际轮速，不能将日志回放当作闭环稳定性证明。

- 保留 .06/.03 转向滞回；改用线性 (abs(error)-.03)/.82，14/18/21 档基础最大减档均为 4。有效强度限制 20%～100%，原设置不覆盖，界面分别显示设置与生效强度。
- 回中减力使用 min(abs(error), max(0, abs(error)+sign(error)*lateralRate*.25))，只降低差速，方向仍取当前观测。减力后 <=.03 允许零差速，其余激活状态至少减一档；保留内轮最低 6、距离选档、身份限档和对准参数。
- 实车与模拟器共用减力和内轮计算。control_submit 增加保存／生效强度、减力后误差、实际减档和协议目标左右轮速；原始误差仍保留。目标轮速映射包含 14 档 240 mm/s、21 档 600 mm/s 及原地转动限制，标注 protocol_target_not_measured，提交不等于底盘执行。
- 数值回放包含正负误差、回中与方向翻转及档位变化；补充强度保留与协议映射回归。不纳入私人图像或原始采集日志。

交付边界：不构建 APK、不安装、不烧录、不提交。后续新包先做直线跟随三次，再做左右缓弯各三次，检查误差不逐次反向扩大且仍有早期差速。实车通过后再提交 OpenBot 与主仓库子模块指针。现有旧包不含本轮修复。

验证结果：JDK 17，`:robot:testDebugUnitTest :robot:testReleaseUnitTest --offline` 成功；Debug / Release 各 368 项，0 失败、0 错误、0 跳过。包含本次数值回放、真实控制器回中撤销差速与反向观测、强度保留、旧协议目标速度映射，以及现有多人、身份恢复、急停、失帧停车与对准回归。主／子仓库 git diff --check 通过。未构建 APK，未进行实车验收。

实车反馈更新：用户确认此前振荡问题得到改善，授权提交并推送当前修复。尚未记录完整路线重复验收，不将其表述为全部实车验收通过。此提交作为接入 R3-V1 前的旧协议基线。


## 2026-09-05：R3-V1 三路遥测接入（未烧录联调）

旧协议基线已推送：OpenBot `f1fa5c4`（codex/voice-cart-tts-smoke），主仓库 `81d5fd6`（codex/sync-voice-module）。本节的 R3 接入为基线之后的工作区改动，尚未提交。

- 新增独立 R3Snapshot 与 R3TelemetrySession。仅收到 r3v1 能力才发送 s0 / !R3,100；旧固件保持 s100。配置等待 1 秒后重试一次，再超时或 bad_r3_config 则 !R3,0 / s100 降级。心跳定时器按 250 ms 周期检查期限；遥测不改变运动许可、急停或看门狗逻辑。
- 严格校验 12 字段、16/32 位范围、固定 L/C/R 顺序、状态、-1 无效值和年龄。有效状态年龄超过 200 ms 为不合协议报文，整行丢弃；可用性逐路按 age + 手机接收后经过时间计算，0 mm 合法。ESP32 emit_ms 仅记录，不减手机时钟。重复/旧/半圈歧义序号不刷新接收时间，65535→0 正常接收；重连清空数据、协商及序号状态。
- BLE/USB 共用 ASCII 128 字节（含换行）组包器：缓存半行、连续多行、超长丢弃至换行后重同步。USB 移除提前剥换行与 AsyncTask 拆行，回调顺序交给统一组包器处理，避免原接收路径吞行/乱序。活动传输通道过滤防止混用不同通道序号。
- 主界面接收入口先处理 R3，避免配置 ACK/快照落入旧 !ERR 展示。实车界面分开显示能力、配置状态、逐路距离、状态与年龄；左右标明侧方，无效/过期为 --。降级时同时保留旧最小值显示。所有测距仍 observation_only。
- 诊断版本 9：control_log.csv 的 r3_telemetry 事件记录每次观测到的快照及有效性/配置状态变化，包含 seq、emit_ms、received_ms、L/C/R 原始 mm/status/age/usable、降级原因。旧 range_log.csv 和 frame_log.csv 的最小值字段保持原含义；不把 R3 中路或最小值当人物距离。

验收待办：新 Android 包配合新固件联调，核对中路实际朝向、左右固定侧方安装、单路遮挡/断开、失效与过期显示、断连重连、配置状态；在跟随关闭时先验遥测，再复测旧运动行为。近距离倒计时标定、传感器运动决策和拐角遮挡策略均未启用。本轮不构建 APK、不安装、不烧录；不纳入个人采集日志和图像。

验证结果：JDK 17 下 `:robot:testDebugUnitTest :robot:testReleaseUnitTest --offline` 成功，Debug / Release 各 380 项，0 失败、0 错误、0 跳过。新增覆盖黄金报文、默认 MTU 分片、多行、128 字节边界、非法状态/数值、200 ms 边界、计时/序号回绕、重复旧包、配置重试降级、重连、Vehicle 协议分流及 USB 原始回调保序；原跟随和停车回归通过。主/子仓库 diff --check 通过。测试不是传感器实物或蓝牙实链路联调。


## 2026-09-05：近距离宽度跟随与有限货架探行 shopping-close-corner-v1

用户已确认 R3 新包/新固件联调成功，并明确授权目标遮挡后的有条件低速前进。本轮实现独立 ShoppingFollowController，通过 RealCartSafetyController 的前台、连接、解锁、当前会话、相机时效、急停和视觉中央障碍检查后才输出；模拟器传入同样的 R3/转角证据时复用同一控制器。旧固件继续走原控制逻辑。初始化仍为原全身身份采集及视觉参考流程，无新增近距采集步骤。

候选规则集中在 ShoppingFollowController（版本 shopping-close-corner-v1）：

- 运行时改用正向画面宽度 EMA（.35）与 45%/55% 停启滞回；远处可选18/21档，30%～45%最高14档，45%以后8档靠近。起步最多8档，每3个新观测至多升2档。宽度单帧缩小25%以上冻结追近500ms，左右裁切/身份外观过渡不追近，上下裁切不影响宽度合法性。身份和轨迹绑定检查继续有效。
- 原转向.06/.03与回中减力复用。边缘余量小于12%时最高10档；小于5%时优先有界原地对准。对准仍使用原600ms与650ms停稳规则，调度器超时独立停车。近距离和探行期间抑制外观学习，避免裁切/遮挡污染身份模板。
- R3 中央300mm取消前进，400mm以上连续3个新快照解除；300～400最高6档，400～500最高10档。无效/过期取消前进，转弯方向对应侧无效或小于250mm限制转动，中央不超过200mm禁止旋转。所有距离从传感器起算，200mm车头净空仍需实测停车余量，不声称软件阈值保证实体净空。
- 界面增加安装标记“左右传感器已固定朝斜前45°（开启沿边探行）”，默认关闭，运行中不能更改；高度不变。未标记时仍可宽度跟随与有限原地搜索，但不能遮挡前进。
- 拐角候选由单人目标连续向外移动、近期侧墙稳定记录及距离增大250mm，或前方小于650mm且目标侧有效空间明显更大支撑。候选500ms过期；需要曾实际产生过前进命令。若侧方变无效，仅允许近期有效间距历史支撑的小角度观察，不将失效等同开口。
- 遮挡后先停650ms，随后每段最多250ms，段间至少150ms且必须有截止后的新画面和新R3快照。以左右镜像的8档外轮与受侧距/前距影响的内轮（通常2～7档）生成紧弧线，不反转前进内轮；转角超过60度且侧距350～650mm时转为沿边段。总计最多6秒或100度。无拐角证据仅原地搜索，最多2.5秒或35度。无效数据不能授权前进。
- 多人/竞争候选、陀螺仪失效、错误转向、超过限制结束探行；原轨迹重新通过现有身份许可后，还需3个新观测才恢复宽度跟随。探行权限不是新人物身份许可。
- 诊断版本10；shopping_policy 事件记录阶段、宽度、安装朝向、侧墙基准、候选依据、方向、航向和实际输出原因。原协议最小值列仍仅观测，R3列标明参与新策略。命令输出、按时间估计的动作范围不是实测轮速/里程。

测试顺序：先静止人员的靠近停车，确认实际车头净空；再直线和左右缓弯；传感器实际转至45度并标记后测试明确单个货架直角；最后测试货架空隙、旁人交叉和单路失效。每项左右/相同路线各三次，优先记录 width、phase、range和command输出。现有三路在9cm高度只观察低处有限区域，不代表完整货架/车身扫掠空间；不保证所有超市拐角可通过。

本轮不构建 APK、不安装、不烧录、不提交；新代码需新包实测。R3能力可用不等于传感器安装已调至45度，请实际调角后再打开对应安装标记。

验证结果：JDK17，`:robot:testDebugUnitTest :robot:testReleaseUnitTest --offline` 成功；Debug / Release 各395项，0失败、0错误、0跳过。新增15项覆盖宽度保持、突然缩窄、前方分级限速/新快照解除、左右镜像拐角、独立动作段截止、安装未确认不前进、未知测距不当开口、多人/陀螺仪/失帧/断连、候选过期与3帧原目标重捕获，以及实车/模拟器输出一致。主/子仓库diff --check通过。以上均非实车拐角通过证明。

## 2026-09-05：R3 无效回波误停车修复 shopping-visible-range-v2

实车日志显示目标持续可靠可见、中央有效距离始终为 910～2380 mm，未出现 300 mm 内近障碍，但 Android 提交的 816 条命令全部为 `c0,0`。左右斜前传感器始终为状态 1，中央后段为状态 2，旧策略将这些无效回波分别解释成“不能差速”和“不能前进”，造成 `range_turn_blocked` 及 `range_front_stop`。日志中的 `!ERR,settling` 是录制开始约 41 秒前的历史错误，不是本次命令拒绝。

按实车约定，目标身份可靠、当前画面新鲜且处于正常可见跟随时，R3 状态 1/2 视为未发现明显障碍，不再限档，也不阻止直行、差速弧线或有界原地对准。有效中央距离仍按 300/400 mm 锁存与三次净空解除；有效侧距小于 250 mm 同样锁存，不能被随后无效回波清除。R3 快照超过 400 ms、状态 4/5、身份或相机失效仍停车。目标遮挡后的拐角探行不采用无效即净空的假设，继续要求有效三路证据。

诊断版本升级到 11。`shopping_policy` 现在同时记录策略版本、决策时刻、R3 序号与接收时刻、L/C/R 的 mm/status/age/决策时可用性、中央/两侧近障碍锁存、净空计数、实际前向限档、无效回波采用方式和最终输出原因。独立 `r3_telemetry` 原始快照日志继续保留。

验证结果：`:robot:testDebugUnitTest :robot:testReleaseUnitTest --offline` 成功，Debug / Release 各 397 项，共 794 项，0 失败、0 错误、0 跳过。新增回归直接覆盖本次日志的 L/R 状态 1、中央 1600 mm、横向误差 -0.10，以及中央状态 2 后恢复正常视觉档位；另验证有效近障碍锁存不会被连续无效回波解除。未构建或安装 APK，未烧录固件。

## 2026-09-09：自动后退完成人物全身取景与初始化

确认身份照片后新增 `AUTO_POSITIONING`：同一已确认轨迹、单个高置信人物在至少 3 个观测和 500 ms 内保持稳定后，若正向画面中的人物框任一边距不足 1%，实车与模拟器统一输出 `c-8,-8`。倒车后不再要求尺度稳定；人物完整入镜的下一次决策立即输出 `c0,0`，停稳 200 ms 后才进入原有 15 个样本、覆盖至少 500 ms 的视觉参考标定。标定结束仍走身份复核和 3 秒倒计时。

稳定窗口限制中心横纵跨度各不超过画面 4%，宽高变化不超过中位值 10%。漏检、竞争人物、轨迹变化、400 ms 相机时效、连接/固件/前台/Start/急停异常均停止倒车。累计倒车上限 6 秒，中断不清零；超时返回确认界面。标定中单帧裁切暂停采样，持续 500 ms 则清空样本并重新自动取景。本轮按授权不使用 R3 限制后退，也不加入后方避障。

诊断版本升级到 12，记录自动取景阶段、轨迹、稳定帧和跨度、正向框与四边边距/裁切、倒车累计及开始/结束时刻、完整入镜时刻、实际控制输出和停车提交延迟；界面及语音改为“保持站立—自动后退—停车标定—3 秒倒计时”，移除要求用户自行后退的提示。

验证结果：JDK 17 下 `:robot:testDebugUnitTest` 与 `:robot:testReleaseUnitTest` 成功，Debug / Release 各 404 项，0 失败。新增覆盖 0/90/180/270 度四边判定、稳定/走动/多人/换轨、完整入镜立即停车、固定 8 档倒车、累计 6 秒上限、实车调度保持/停止、模拟器一致性与失帧停车、标定持续裁切回退；既有身份恢复、R3 跟随、丢失和安全回归通过。未构建或安装 APK，未烧录固件，未纳入私人日志。
## 2026-09-10：ShoppingCart 发行入口、即时语音与自动取景实车阻断修复

- 新增与 `Real Cart Follow` 并列的 `ShoppingCart` 入口。页面保留相机预览、蓝牙连接、急停和中央大号 Start，确认阶段临时显示“确认跟随／重新采集”照片卡片；遥控、模式、协议、R3 数值、模型、置信度和日志控件不进入发行布局。
- ShoppingCart 直接调用共享启动与停车入口：启动前检查 BLE、固件和检测器，自动切换 AUTO、解除普通自动锁并启动采集；后台、断连、Start 关闭和急停继续走同一安全控制器。发行配置固定最高档 `c21`，不会覆盖调试入口保存的档位，距离、转弯和安全限档仍可降低输出。
- ShoppingCart 内部诊断默认开启，保留 CSV、事件、crop、gallery 和 scene，`app_mode` 标记为 `ShoppingCart`；隐藏日志开关不会改变内部状态。日志版本升为 13。
- 语音改为结构化阶段驱动，自动取景按等待稳定、倒车、全身入镜并初始化、倒计时播报；不再解析中文诊断字符串。所有新提示都使用 `QUEUE_FLUSH`，TTS 延迟就绪只保留最新提示，急停／断连／用户停止锁存后不会被迟到帧覆盖。
- 补齐 `AUTO_POSITIONING` 与通用身份恢复分支的隔离，保证自动取景先于正常身份恢复门控执行。
- 手机最新会话 `cart_diag_20260910_101016_015_c76d770e` 显示自动取景、15 帧视觉标定、三帧身份复核均完成且已进入 `FOLLOW`；未启动的直接原因是 R3 持续为 `L=SIGNAL_INVALID, C=SIGNAL_INVALID, R=NOT_PRESENT` 时被误判为 `range_sensor_fault`，每帧覆盖为 `c0,0`。
- 策略升级为 `shopping-visible-range-v3`：新鲜快照中的状态 1/2/3/5 在目标可见、身份和相机均有效时表示“没有可用障碍证据”，允许视觉跟随；只有状态 4 `BUS_ERROR` 作为明确传感器故障停车。已有近障碍锁存仍不会被无效回波解除，目标遮挡后的沿边探行仍要求有效测距。
- 增加发行布局、固定档位、结构化语音去重与切换、无回波/未安装可见跟随及 BUS_ERROR 停车回归。按交付边界仅运行 Debug/Release 单元测试，不构建 APK、不安装、不烧录。

## 2026-09-10：ShoppingCart 蓝牙返回、Start 就绪与欢迎语音修复

实机日志确认，从 ShoppingCart 打开蓝牙页会按生命周期释放检测器；返回后检测器需要由新相机帧异步重建。旧实现让 Start 直接做一次检测器检查，因此重建期间只提示“相机模型正在准备”，不会在稍后自动反映为可用。同时，ShoppingCart 每次 `onResume` 都播放语音模拟器欢迎词，蓝牙页返回会重复播报。另有三次 `RejectedExecutionException` 明确来自页面销毁后的窗口失焦回调向已关闭的转向调参记录器写入。

- 新增统一的发行启动就绪状态，按急停、BLE、固件握手、模型加载/错误、相机新鲜度和 READY 排序。状态栏、Start 可用性与实际启动入口共用同一判断；返回蓝牙页后按钮保持禁用，检测器与新画面就绪后自动启用，但不会自动开始运动。运行中的 Start 始终可用于停车。
- ShoppingCart 使用独立欢迎词，未连接时先引导连接蓝牙；已连接时提示等待准备后点击 Start。欢迎状态保存到当前导航栈条目的 `SavedStateHandle`，蓝牙返回、后台恢复和旋转不重播，新进入 ShoppingCart 才重新播报。语音模拟器保留原欢迎词，TTS 恢复时不再隐式播放欢迎词。
- 窗口焦点监听器在 `onDestroyView` 中先移除，转向记录器改为视图生命周期并幂等关闭；关闭后的迟到记录直接忽略，避免页面切换或 Activity 销毁崩溃。固件握手等待不再误判成 BLE 断开语音。

验证结果：JDK 17 下 `:robot:testDebugUnitTest` 与 `:robot:testReleaseUnitTest` 成功，Debug / Release 各 413 项，0 失败、0 错误、0 跳过。新增覆盖启动就绪顺序、模型加载和错误状态、按钮自动启用、单导航条目欢迎去重，以及记录器重复关闭和关闭后迟到写入。未构建或安装 APK，未提交手机私人日志。

## 2026-09-10：ShoppingCart 相机模型永久等待修复

手机现场确认 BLE、R3、相机权限、1920×1080 预览和 1280×720 ImageAnalysis 均正常，保存模型为内置 `MobileNetV1-300.tflite`；分析帧持续产生，但 `inference` Looper 空闲。根因是首个分析帧可能早于 Fragment `onResume()` 到达：旧代码先设置模型配置 pending，再向尚未创建的 Handler 提交任务；提交被静默丢弃后，后续帧因 pending 不再重试。

- 推理任务提交现在返回是否成功。Handler 不存在、退出或拒绝任务时立即撤销 pending，下一帧继续尝试；`onResume()` 创建推理线程后还会使用已缓存的帧尺寸主动补发。
- 模型创建、位图和坐标矩阵配置统一由 `finally` 清理 pending 和临时候选 Detector。模型错误进入明确错误状态，不再无限显示“相机模型正在准备”。
- 新增 5 秒加载期限：第一次超时使旧 generation 失效并重试一次，第二次超时显示“模型准备失败，请退出后重新进入”。页面暂停会清除相机新鲜度，恢复后必须收到新画面才可启动。
- 发行版就绪顺序改为先判断相机画面、再判断模型；模型真正就绪后 Start 自动启用但不会自动开始。`CartInferenceStartup` 记录提交、拒绝、恢复补发、超时、错误和成功事件，不记录图像。
- 新增纯状态机测试覆盖提交拒绝、完成清理、5 秒边界、一次重试、最终失败和会话重置，并更新 ShoppingCart 就绪提示测试。

验证结果：JDK 17 下 `:robot:testDebugUnitTest :robot:testReleaseUnitTest --offline` 成功，Debug / Release 各 417 项，0 失败、0 错误、0 跳过；`git diff --check` 通过。未构建或安装 APK，未提交或推送。

### 隐藏模型控件下的显式初始化补充

继续复测发现手机仍运行 11:42 安装的旧包，同时原实现仍依赖模型 Spinner 的 `onItemSelected` 回调设置 `model`。ShoppingCart 会隐藏模型控件及父容器，因而该回调不能作为发行入口的初始化前提。

- 页面创建时直接从检测模型清单解析保存值并设置模型；保存值为空或失效时回退内置 `MobileNetV1-300.tflite`。清单或内置模型缺失时显示可执行的错误提示，不再静默停留在 loading。
- Spinner 只负责显示和后续人工切换；同一名称、路径及类型的模型不会重复触发网络重建。
- 模型加载器增加测试替换点，回归测试实际经过“模型解析—任务提交—推理资源建立—READY”。覆盖隐藏控件、首帧早于 Handler、提交拒绝后恢复、加载失败、保存值有效／空／失效和视图重建。
- 过期 generation 退出时只由 `finally` 释放候选 Detector，消除重复关闭。

验证结果：JDK 17 下 Debug / Release 单元测试各 424 项，0 失败、0 错误、0 跳过；`git diff --check` 通过。用户随后授权构建并安装到当前手机，安装与实机启动结果另行记录。

## 2026-09-10：ShoppingCart 采集完成后自动确认

ShoppingCart 不再要求用户在采集结束后走到手机旁确认照片。达到 15 个有效采集观测的同一推理帧，发行入口使用该帧的准确轨迹 ID 锁定目标、确认 ReID Gallery、保存诊断快照并进入 `AUTO_POSITIONING`；输出保持停车，下一帧才开始稳定性与完整入镜判断。若准确轨迹已失效，则放弃本次结果并重新采集，不允许就近选择其他人物。Real Cart Follow 与模拟器继续保留人工确认。

发行页面永久隐藏确认卡片及确认／重拍按钮。自动取景累计倒车达到 6 秒时，ShoppingCart 记录 `positioning_timeout_stop`、停车、关闭本次 Start 并提示用户调整站位后重新点击 Start，不再返回隐藏的确认状态。自动确认成功和失败分别记录 `target_auto_confirmed` 与 `target_auto_confirm_failed`，确认快照仍进入默认开启的本地诊断。

ShoppingCart 语音改为自动流程专用序列：采集目标、采集完成并保持站立、自动后退、停车完成初始化、三秒后开始跟随。发行流程不再播报“请在手机上确认照片”；超时明确播报小车已停止并要求重新点击 Start。语音仍使用 `QUEUE_FLUSH`，普通调试入口保留原人工确认提示。

验证结果：JDK 17 下 `:robot:testDebugUnitTest :robot:testReleaseUnitTest --offline` 成功，Debug / Release 各 430 项，0 失败、0 错误、0 跳过；`git diff --check` 通过。新增覆盖同帧精确轨迹确认、失效轨迹重新采集、调试入口保留人工确认、发行确认面板隐藏、采集完成即时状态、超时文案及自动流程语音。用户随后授权构建并确认 Debug APK 安装完成，手机包更新时间为 2026-09-10 13:26:02，应用已启动；未提交、未推送。

## 2026-09-10：缩短跟随距离与离画新轨迹恢复 shopping-close-return-v4

手机最近两段 ShoppingCart 会话已导出到本机临时目录供离线复盘，私人图片和日志不进入仓库。正常前进时人物宽度平均约为画面 24%，不少决策已经输出 21 档；目标离画返回后，全局 ReID 曾累计 3 次强匹配，但后续 0.55～0.74 的分数波动清空进度。另确认身份层授权新轨迹后，购物车运动控制器仍只接受旧 `seenTrack`，会继续停车。

- 距离策略版本升级为 `shopping-close-return-v4`。宽度滞回改为 55% 恢复追近、65% 停止；可靠目标按宽度小于 40%／50%／55%／60%／65% 分别请求 21／18／14／10／8 档。左右裁切、尺度突变、身份谨慎、画面边缘和 R3 近障碍限制继续优先，起步与每 3 个新观测最多升 2 档的限制不变。上下裁切仍不直接否定宽度控制。
- 全局身份恢复使用最多 2 秒的证据窗口，仍需 5 次独立新鲜强匹配且跨度至少 1.2 秒。0.70～0.85 的新鲜分数暂停累计；缓存结果不计数也不单独清空；连续两次低于 0.70、候选换轨、多人／关联歧义或 500 ms 内无有效身份观测会清空窗口。验证期间保持停车，未知候选不能写入 Gallery。
- 身份层明确授权的新轨迹可以替换旧运动轨迹。交接时清除旧宽度、升档、搜索、货架开口和转弯上下文，保留 R3 近障碍锁存；新轨迹再连续通过 3 个新鲜身份允许观测后才恢复跟随，不能继承原目标的拐角探行权限。
- ShoppingCart 对单个核验候选显示黄色“正在确认目标”，低置信观测显示灰色“目标检测不稳定”。这些框只说明检测／核验状态，不赋予运动权限。诊断版本升级到 14，新增原始与平滑宽度、启停阈值、期望档位、边缘／身份／谨慎／中央测距／配置限档、最终轮速档位、升档重置原因和轨迹交接进度。
- 旧日志中的 696 条 `shopping_policy` 宽度跟随记录按新分档离线重算，642 条请求 21 档、47 条请求 18 档、7 条请求 14 档。该回放验证输出规则，并能在后续实测中区分软件限速与已经请求最高档仍追不上的底盘／执行链路问题，不代表实车距离已经达标。

验证结果：JDK 17 下 `:robot:testDebugUnitTest :robot:testReleaseUnitTest --offline` 成功，Debug / Release 各 439 项，0 失败、0 错误、0 跳过；`git diff --check` 通过。新增覆盖 55%／65% 边界、左右裁切、新宽度五档区间、中等分数暂停、缓存不计数、两次低分清空、2 秒窗口、全局身份层到实车安全控制器的新轨迹交接、3 帧恢复及发行候选框文案。未构建、未安装、未提交、未推送。

## 2026-09-10：初始化连续确认与多人原目标恢复

最新两段 ShoppingCart 会话确认，自动取景和 15 帧视觉参考标定实际已经完成；启动停在身份复核，是因为同一确认轨迹在取景后的 ReID 分数约 0.82～0.84，仍被旧的 0.85 强匹配门槛反复清零。多人会话中，锁定轨迹、可见状态和独立 bbox 几何均连续，但连续跟踪器的一次 `bbox_jump` 覆盖了已有局部证据；多人检查超时还会直接清空原目标上下文。

- 自动确认后建立绑定会话和准确轨迹的初始化连续记录，贯穿自动取景、停稳、标定和三秒倒计时。原目标连续三个新鲜高置信观测通过几何检查后，标定完成可直接进入倒计时，不伪造强 ReID；缺失、换轨、关联竞争、多人或超过 500 ms 的观测间隔会使该权限失效并回到严格身份复核。
- 自动后退完成后的停稳和标定阶段，每 200 ms 最多从同一确认轨迹补充一个完整、未裁切外观，最多八个；该入口不要求与旧模板达到 0.85，相似度结果也不参与本次初始化身份证明。样本记录来源帧、轨迹、会话和 `initialization_continuity` 来源，换轨、重复帧、过期帧、跨会话及初始化结束后的写入均拒绝。
- ShoppingCart 初始化阶段隐藏低置信灰框，低置信身体局部框不参与稳定、完整入镜或身份候选选择；真实高置信竞争人物仍会中断初始化连续权限。
- 多人跟随把运动许可与原目标上下文分开。旁人出现和单次低 ReID 不切换目标；独立几何仍支持同一轨迹时，连续跟踪器单次抖动不再清空上下文。多人检查缺少结论超过一秒时停车但保留原轨迹，旁人离开后可恢复；明确身份冲突、真实空间跳变或关联竞争仍进入严格全局验证。
- ShoppingCart 状态栏和语音在 FOLLOW/FOLLOW_CAUTION 状态但身份运动许可被撤销时显示／播报“正在确认目标”，不再误报“正在跟随”。诊断版本升级到 15，连续权限、初始化补样和身份许可变化均写独立事件，不受普通帧抽样间隔限制。

验证结果：JDK 17 下 `:robot:testDebugUnitTest :robot:testReleaseUnitTest` 成功，Debug / Release 各 446 项，0 失败、0 错误、0 跳过；`git diff --check` 通过。回归覆盖连续初始化直达倒计时、目标真实缺失、补样轨迹／会话边界、初始化灰框隐藏、单次跟踪抖动、多人成员加入及超时停车后恢复、真实空间跳变严格复核、状态栏和语音许可提示。本轮未构建或安装 APK，未提交或推送，私人日志与图片未加入仓库。
